...
RECIPIENT = "yelohuang@gmail.com"


def fetch_trending_terms() -> list[str]:
    """Return today's top US trending search terms."""
...
def build_email_html(matches: list[dict]) -> str:
    """Build HTML email body from a list of match dicts."""
    today = date.today().strftime("%B %d, %Y")

    if not matches:
        return f"""<!DOCTYPE html>
<html><body style="font-family:Arial,sans-serif">
<h2>Stock Trend Radar &mdash; {today}</h2>
<p>No S&amp;P 500 stocks found in today's trending searches.</p>
</body></html>"""
...
def run() -> None:
    ...
    today = date.today().strftime("%B %d, %Y")
    send_email(f"Stock Trend Radar - {today}", build_email_html(matches))
